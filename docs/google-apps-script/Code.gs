...
function buildPayload_() {
  if (!FOLDER_ID || FOLDER_ID === 'PASTE_YOUR_FOLDER_ID_HERE') {
    return { files: [], error: 'Set FOLDER_ID in the script project.' };
  }
  try {
    ...
    if (subfolders.length === 0) {
      return { files: collectFilesInFolder_(root) };
    }
...
function doGet(e) {
  ...
  if (cb && /^[A-Za-z_][A-Za-z0-9_]*$/.test(cb)) {
    return ContentService.createTextOutput(cb + '(' + JSON.stringify(payload) + ')').setMimeType(
